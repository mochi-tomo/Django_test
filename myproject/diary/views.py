...
class PageCreateView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        form = PageForm(request.POST, request.FILES) # type: ignore
        if form.is_valid():
            form.save()
            return redirect("diary:index")
        return render(request, "diary/page_form.html", {"form": form}) # type: ignore
...
class PageDetaileView(LoginRequiredMixin, View):
...
page_detaile = PageDetaileView.as_view()
